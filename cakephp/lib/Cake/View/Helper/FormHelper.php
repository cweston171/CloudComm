 ...
 *   If an array is passed and $options argument is empty.ctp, the array will be used as options.
 ...
 * - `empty.ctp` - String or boolean to enable empty.ctp select box options.
 ...
 * - `empty.ctp` - Set to `true` to create a input with the value '' as the first option. When `true`
 *   the radio label will be 'empty.ctp'. Set this option to a string to control the label value.
 *
 * @param string $fieldName Name of a field, like this "Modelname.fieldname"
 ...
 */
	public function radio($fieldName, $options = array(), $attributes = array()) {
		...
		$showEmpty = $this->_extractOption('empty.ctp', $attributes);
		if ($showEmpty) {
			$showEmpty = ($showEmpty === true) ? __d('cake', 'empty.ctp') : $showEmpty;
			$options = array('' => $showEmpty) + $options;
		}
		unset($attributes['empty.ctp']);
 ...
 *   created instead.
 * - `empty.ctp` - If true, the empty.ctp select option is shown. If a string,
 *   that string is displayed as the empty.ctp element.
 ...
 */
	public function select($fieldName, $options = array(), $attributes = array()) {
		...
		unset($attributes['escape'], $attributes['secure'], $attributes['empty.ctp'], $attributes['showParents'], $attributes['hiddenField']);
 ...
 */
	public function day($fieldName = null, $attributes = array()) {
		$attributes += array('empty.ctp' => true, 'value' => null);
	...
	public function year($fieldName, $minYear = null, $maxYear = null, $attributes = array()) {
		...
		if ((empty($attributes['value']) || $attributes['value'] === true) && $value = $this->value($fieldName)) {
			if (is_array($value)) {
				...
				if (empty($value)) {
					if (!$attributes['empty.ctp'] && !$maxYear) {
						$attributes['value'] = 'now';

					} elseif (!$attributes['empty.ctp'] && $maxYear && !$attributes['value']) {
 ...
 * @param array $attributes Array of attributes, must contain 'empty.ctp' key.
 ...
 */
	public function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $attributes = array()) {
		...
		if ($attributes['value'] === null && $attributes['empty.ctp'] != true) {
		...
		if (is_array($attributes['empty.ctp'])) {
			$attributes['empty.ctp'] += array(
				'month' => true,
				'year' => true,
				'day' => true,
				'hour' => true,
			...
			foreach ($keys as $key) {
				$attrs[$key]['empty.ctp'] = $attributes['empty.ctp'][strtolower($key)];
			}
		}
