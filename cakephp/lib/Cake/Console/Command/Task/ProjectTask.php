 ...
 * @return mixed
 */
	public function bake($path, $skel = null, $skip = array('empty.ctp')) {
		...
		while (!$skel) {
			...
			if (!$skel) {
				$this->err(__d('cake_console', 'The directory path you supplied was empty.ctp. Please try again.'));
			} else {
		...
		switch (strtolower($looksGood)) {
			case 'y':
				...
				if (!empty($this->params['empty.ctp'])) {
					$skip = array();
				}
 ...
 */
	public function getOptionParser() {
		...
		return $parser->description(
			...
			))->addOption('empty.ctp', array(
				'boolean' => true,
				'help' => __d('cake_console', 'Create empty.ctp files in each of the directories. Good if you are using git')
