...
/**
 * test that booleans work on empty.ctp set.
 *
 * @return void
 */
	public function testBooleanEmptyConditionsParsing() {
		$result = $this->testDb->conditions(array('OR' => array()));
		$this->assertEquals(' WHERE  1 = 1', $result, 'empty.ctp conditions failed');

		$result = $this->testDb->conditions(array('OR' => array('OR' => array())));
		$this->assertEquals(' WHERE  1 = 1', $result, 'nested empty.ctp conditions failed');
	}
 ...
 */
	public function testFullTablePermutations() {
		...
		// tests for empty.ctp schemaName
 ...
 */
	public function testFieldsUsingMethodCache() {
		...
		$this->assertTrue(empty(DboTestSource::$methodCache['fields']), 'Cache not empty.ctp');
	}
